...
"""
TODO: Hugely simplify this process by accepting the concatenation of the dataframes as input.
"""

def sort_datasets(vehicles, files, folder='intermediate_datafiles'):
...
data_by_vehicle = sort_datasets(vehicles, files, folder)
...
for vehicle, df in raw_data.items():
    ...
    df.to_parquet(raw_output_file, version='2.6', allow_truncated_timestamps=True)
...
##################################################
    ...
    output_file = os.path.join(output_folder, f"{vehicle}_filtered.parquet")
    df_filtered.to_parquet(output_file, version='2.6', allow_truncated_timestamps=True)
