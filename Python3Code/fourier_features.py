...
def drop_shit(df):
    bad_cols = [col for col in df.columns if 'location' in str(col).lower()] + ['proximity_phone_Distance']
    return df.drop(columns=[col for col in bad_cols if col in df.columns])
...
def plot_and_save_spectrum(df, signal_prefix, base_filename, recording_id=0, out_dir='figures/fourier'):
    ...
    cols = [col for col in df.columns 
            if re.match(rf"^{signal_prefix}_freq_\d+\.?\d*_Hz", col)]
    ...
    plt.plot(freqs, spectrum.values)
...
df = pd.read_csv(dataset_name, index_col=0, parse_dates=True)
df = drop_shit(df)
df = add_recording_index(df)
df = df.dropna()
grouped = df.groupby('recording_number')
...
# Generate and save spectrum plots
